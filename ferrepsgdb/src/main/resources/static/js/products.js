...
const URL_MAIN = 'http://localhost:8080/api/productos/'; //URL a donde se hace la petición
...
function view(index) {
	//index es el índice del producto que se va a ver
	// console.log(index);
	// console.table(productos[index]);
	document.getElementById('productTitleModal').innerHTML =
		productos[index].nombre; //se cambia el título del modal
	document.getElementById(
		'productBodyModal'
	).innerHTML = `${productos[index].descripcion}  <img class="bd-placeholder-img card-img-top" role="img" src="img/${productos[index].url_Imagen}" />
...
*/
fetch(URL_MAIN, {
	//URL del servicio a donde se hara el POST
	method: 'POST', // or 'PUT'
	headers: {
		// se agrega el header
